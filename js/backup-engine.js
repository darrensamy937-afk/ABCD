...
(function (global) {
    ...
    /**
     * 收集 MomentsVideoDB（朋友圈大图片/视频的独立 IndexedDB）中的所有数据
     * 返回 { images: {key: value}, videos: {key: value} } 或 null
     */
    async function collectMomentsIDB() {
        try {
            ...
            function readAllKeys(storeName) {
            ...
            function readValues(storeName, keys) {
                return Promise.all(keys.map(function (key) {
                    return new Promise(function (resolve) {
                        try {
                            var tx = db.transaction(storeName, 'readonly');
                            var req = tx.objectStore(storeName).get(key);
                            req.onsuccess = function () { resolve({ key: key, val: req.result }); };
                            req.onerror = function () { resolve({ key: key, val: null }); };
                        } catch (e) { resolve({ key: key, val: null }); }
                    });
                }));
            }
            ...
            var imgKeys = await readAllKeys('images');
            if (imgKeys.length > 0) {
                var imgPairs = await readValues('images', imgKeys);
                for (var i = 0; i < imgPairs.length; i++) {
                    if (imgPairs[i].val != null) result.images[imgPairs[i].key] = imgPairs[i].val;
                }
            }

            var vidKeys = await readAllKeys('videos');
            if (vidKeys.length > 0) {
                var vidPairs = await readValues('videos', vidKeys);
                for (var j = 0; j < vidPairs.length; j++) {
                    if (vidPairs[j].val != null) result.videos[vidPairs[j].key] = vidPairs[j].val;
                }
            }

            db.close();
            var totalKeys = Object.keys(result.images).length + Object.keys(result.videos).length;
            if (totalKeys === 0) return null;
            return result;
        } catch (e) {
            console.warn('[backup] 收集 MomentsVideoDB 失败:', e);
            return null;
        }
    }

    /**
     * 将备份中的 MomentsVideoDB 数据恢复到 IndexedDB
     */
    async function restoreMomentsIDB(data) {
        var midb = data.momentsIDB;
        if (!midb || typeof midb !== 'object') return;
        try {
            var db = await new Promise(function (resolve, reject) {
                var req = indexedDB.open('MomentsVideoDB', 2);
                req.onupgradeneeded = function (e) {
            ...
            function writeStore(storeName, entries) {
                return new Promise(function (resolve) {
                    if (!entries || Object.keys(entries).length === 0) { resolve(); return; }
                    try {
            ...
            await writeStore('images', midb.images);
            await writeStore('videos', midb.videos);
            db.close();
            console.log('[backup] MomentsVideoDB 恢复完成, images:', Object.keys(midb.images || {}).length, 'videos:', Object.keys(midb.videos || {}).length);
        } catch (e) {
            console.warn('[backup] 恢复 MomentsVideoDB 失败:', e);
        }
    }
    ...
    async function buildBackupPayload(flags) {
        ...
        // 收集 MomentsVideoDB（朋友圈大图片/视频的独立 IndexedDB）
        var momentsIDBRaw = null;
        if (flags.inclMoments !== false) {
            momentsIDBRaw = await collectMomentsIDB();
        }
        ...
        // 对 MomentsVideoDB 数据也做媒体提取（减小备份体积）
        var momentsIDBOut = null;
        if (momentsIDBRaw) {
            momentsIDBOut = extractMediaTree(momentsIDBRaw, state);
        }
        ...
        if (typeof JSZip !== 'undefined') {
            try {
                ...
                var jsonBody = {
                    ...
                    momentsIDB: payload.momentsIDB || null,
    ...
    async function applyBackupToStorage(data, opt) {
        ...
        // 恢复 MomentsVideoDB（朋友圈大图片/视频的独立 IndexedDB）
        // 选择性导入时，仅当选中了 moments 类别才恢复
        var shouldRestoreMomentsIDB = !selective || !opt.selectedCategoryIds || opt.selectedCategoryIds.indexOf('moments') !== -1;
        if (shouldRestoreMomentsIDB && data.momentsIDB) {
            var inlinedMomentsIDB = inlineMediaTree(data.momentsIDB, mediaStore);
            await restoreMomentsIDB({ momentsIDB: inlinedMomentsIDB });
        }
    }
    ...
    global.ChatBackup = {
        ...
        collectMomentsIDB: collectMomentsIDB,
        restoreMomentsIDB: restoreMomentsIDB
    };
