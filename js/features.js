...
var statusPool = [
    ...
    var userSalt = '12345';
    try {
        var savedSalt = localStorage.getItem('_dgUserSalt');
        if (savedSalt) {
            userSalt = savedSalt;
        } else {
            userSalt = String(Math.floor(Math.random() * 999983) + 1);
            try { localStorage.setItem('_dgUserSalt', userSalt); } catch(e) {}
        }
    } catch(e) {}
    var seed = 0;
    ...
    var weather = defaultWeather;
    try {
        var savedWeather = localStorage.getItem(customWeatherKey);
        if (savedWeather) weather = savedWeather;
    } catch(e) {}

    // 混合系统预设 + 用户自定义状态池
    var userStatusPool = [];
    try { 
        var rawPool = localStorage.getItem('dg_status_pool');
        if (rawPool) userStatusPool = JSON.parse(rawPool) || [];
    } catch(e) { userStatusPool = []; }
    var userStatusTexts = [];
    try {
        userStatusTexts = userStatusPool.map(function(item) { return item.status || item; }).filter(Boolean);
    } catch(e) { userStatusTexts = []; }
    var mixedStatusPool = statusPool.concat(userStatusTexts);
    var status = mixedStatusPool[Math.floor(seededRandDg(seed, 1) * mixedStatusPool.length)] || '一切都好';
...
function _buildDailyGreeting() {
    try {
        ...
        try {
            ...
            console.warn('getDailyGreetingData error:', eData);
            data = { timeLabel: '你好', timeEmoji: '✨', festival: null, weather: '晴朗', status: '想你了 💭' };
        }
        var festival = data.festival || null;
        var timeLabel = data.timeLabel || '你好';
        var timeEmoji = data.timeEmoji || '✨';
        var weather = data.weather || '晴朗';
        var status = data.status || '一切都好';

        var now = new Date();
        var todayStr = now.getFullYear() + '-' + String(now.getMonth()+1).padStart(2,'0') + '-' + String(now.getDate()).padStart(2,'0');

        var moodDataRaw = {};
        try { moodDataRaw = window.moodData || {}; } catch(e) {}
        var todayMood = moodDataRaw[todayStr] || null;
        var allMoods = [];
        try {
            if (typeof getAllMoodOptions === 'function') allMoods = getAllMoodOptions() || [];
        } catch(e) {}

        var pName = '梦角';
        try {
            if (typeof settings !== 'undefined' && settings && settings.partnerName) pName = settings.partnerName;
        } catch(e) {}
        var mName = '我';
        try {
            if (typeof settings !== 'undefined' && settings && settings.myName) mName = settings.myName;
        } catch(e) {}
        ...
        if (todayMood && todayMood.partner) {
            try {
                for (var pi = 0; pi < allMoods.length; pi++) {
                    if (allMoods[pi] && allMoods[pi].key === todayMood.partner) {
                        partnerMoodText = (allMoods[pi].kaomoji || '') + '  ' + (allMoods[pi].label || '');
                        partnerMoodIcon = allMoods[pi].kaomoji || null;
                        break;
                    }
                }
                partnerMoodNote = todayMood.partnerNote || '';
            } catch(e) { console.warn('mood parse error:', e); }
        }
        ...
        var customData = {};
        try { 
            var rawCustom = localStorage.getItem('dg_custom_data');
            if (rawCustom) customData = JSON.parse(rawCustom) || {};
        } catch(e2) { customData = {}; }
        ...
        // 安全地合并数组（避免扩展运算符在非数组时报错）
        var mixedTitles = defaultTitles.slice();
        try {
            if (customData.titles && Array.isArray(customData.titles) && customData.titles.length > 0) {
                mixedTitles = customData.titles.concat(defaultTitles);
            } else if (customData.title && typeof customData.title === 'string') {
                mixedTitles = [customData.title].concat(defaultTitles);
            }
        } catch(e) { mixedTitles = defaultTitles.slice(); }

        var mixedNotes = defaultNotes.slice();
        try {
            if (customData.notes && Array.isArray(customData.notes) && customData.notes.length > 0) {
                mixedNotes = customData.notes.concat(defaultNotes);
            } else if (customData.note && typeof customData.note === 'string') {
                mixedNotes = [customData.note].concat(defaultNotes);
            }
        } catch(e) { mixedNotes = defaultNotes.slice(); }

        try {
            mainTitle = mixedTitles[Math.floor(seededRandom(todaySeedForText) * mixedTitles.length)] || mainTitle;
            noteText = mixedNotes[Math.floor(seededRandom(todaySeedForText + 1) * mixedNotes.length)] || noteText;
        } catch(e) {}

        function setEl(id, val) { try { var el = document.getElementById(id); if (el) el.textContent = val; } catch(e) {} }
        function setElHTML(id, val) { try { var el = document.getElementById(id); if (el) el.innerHTML = val; } catch(e) {} }

        var emojiEl = document.getElementById('dg-emoji');
        if (emojiEl) {
            try {
                if (festival && festival.emoji) {
        ...
        var statusPoolData = [];
        try { 
            var rawStatus = localStorage.getItem('dg_status_pool');
            if (rawStatus) statusPoolData = JSON.parse(rawStatus) || [];
        } catch(e2) { statusPoolData = []; }
        
        // 将系统预设 + 用户自定义混合后，按今日种子选取
        var systemStatusItems = [];
        try {
            var baseStatus = (typeof status !== 'undefined') ? status : '';
            if (baseStatus) systemStatusItems.push({ status: baseStatus, icon: null, iconImg: null });
        } catch(e) {}
        ...
        if (fullPool.length > 0) {
            ...
            } catch(e) { console.warn('status pool error:', e); setEl('dg-status', status); }
        } else {
        ...
        try {
            ...
            if (overlayBg && typeof applyDgOverlayBg === 'function') {
                applyDgOverlayBg(overlayBg);
            }
        } catch(e) {}
...
// 备用极简公告
window.showFallbackGreeting = function() {
    window.tryShowDailyGreeting();
};
...
window.tryShowDailyGreeting = function() {
    ...
    // 随机选择公告内容
    var titles = ['今日宜', '今日忌', '今日提示', '今日运势', '今日心情'];
    var contents = [
        '宜：一起看日落，一起吃火锅，一起听歌',
        '宜：多喝热水，早睡早起，想你',
        '忌：冷战，不回消息，独自难过',
    ...
    var labels = ['MORNING', 'DAILY', 'LOVE', 'CARE', 'BLESS'];
    var icons = ['☀️', '🌸', '💌', '🌙', '⭐'];
    ...
    var contentEl = document.getElementById('dg-content');
    var labelEl = document.getElementById('dg-label');
    var iconEl = document.getElementById('dg-icon');
    var dateEl = document.getElementById('dg-date-stamp');
    ...
    if (dateEl) {
        var now = new Date();
        dateEl.textContent = '— ' + (now.getMonth()+1) + '月' + now.getDate() + '日 公告已送达 —';
    }
...
// 公告事件绑定
(function initDailyGreetingEvents() {
    // 点击公告按钮时显示（已在HTML中绑定 onclick）
    // 点击模态框外部关闭
    document.addEventListener('click', function(e) {
        var modal = document.getElementById('daily-greeting-modal');
        if (!modal || modal.classList.contains('hidden')) return;
        if (e.target === modal) {
            window.closeDailyGreeting();
        }
    });
})();
