...
(function() {
    ...
    const defaultAppIcons = {
        ...
        'diary-journal': '<i class="fas fa-book-open"></i>',
        'live-stream': '<svg width="32" height="32" viewBox="0 0 24 24" fill="none" stroke="#0d3b66" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"><rect x="2" y="3" width="20" height="14" rx="2"/><line x1="8" y1="21" x2="16" y2="21"/><line x1="12" y1="17" x2="12" y2="21"/><circle cx="12" cy="10" r="3" fill="#0d3b66" stroke="none" opacity="0.15"/><circle cx="12" cy="10" r="1.5" fill="#0d3b66" stroke="none"/></svg>'
    };

    const defaultAppOrder = ['chat', 'mailbox', 'moyu', 'diary', 'fortune', 'mood', 'calendar', 'decide', 'music-buddy', 'accounting', 'diary-journal', 'live-stream'];
    ...
    function renderIconGrid() {
        ...
        grid.innerHTML = '';
        const nameMap = { chat:'会话', mailbox:'信封', moyu:'摸鱼', diary:'朝夕心记', fortune:'运势', mood:'心晴', calendar:'日历', decide:'抉择', 'music-buddy':'一起听', accounting:'记账', 'diary-journal':'手账日记', 'live-stream':'直播' };
    ...
    window.openApp = function(app) {
        const appActions = {
            ...
            'diary-journal': () => {
                if (window.DiaryJournalApp && typeof window.DiaryJournalApp.open === 'function') {
                    window.DiaryJournalApp.open();
                }
            },
            'live-stream': () => {
                if (window.LiveStreamApp && typeof window.LiveStreamApp.open === 'function') {
                    window.LiveStreamApp.open();
                }
            },
            'moments': () => {
                // 显示朋友圈页面
    ...
    window.hideMomentsPage = function() {
        ...
        // 退出前紧急同步保存朋友圈数据，防止数据丢失
        if (typeof MomentsApp !== 'undefined' && MomentsApp.emergencySyncSave) {
            MomentsApp.emergencySyncSave();
        }
    ...
    async function loadSavedSettings() {
        ...
        if (savedOrder) {
            try {
                appOrder = JSON.parse(savedOrder).filter(a => a && a !== 'map' && a !== 'music-game');
                // 补齐新增的日记入口
                if (!appOrder.includes('diary-journal')) appOrder.push('diary-journal');
                if (!appOrder.includes('live-stream')) appOrder.push('live-stream');
                reorderAppItems();
            } catch(e) {}
        } else {
    ...
    // 隐藏"对方正在输入"指示器的辅助函数（与 core.js 中的模式保持一致）
    function hideTypingIndicatorUI() {
        ...
        if (tiWrapper) {
            var tiInner = tiWrapper.querySelector('.typing-indicator');
            if (tiInner) {
        ...
        if (oldTi) {
            oldTi.style.display = 'none';
            oldTi.classList.remove('show');
        }
    }
    ...
    function clearSessionTimers() {
        // 清除自动发送定时器
        if (typeof autoSendTimer !== 'undefined' && autoSendTimer) {
            clearTimeout(autoSendTimer);
            autoSendTimer = null;
        }
        // 清除"已读不回复"定时器
        if (typeof readNoReplyTimer !== 'undefined' && readNoReplyTimer) {
            clearTimeout(readNoReplyTimer);
            readNoReplyTimer = null;
        }
        // 清除保存防抖定时器（防止延迟保存将旧数据写入新会话）
    ...
    async function switchObjectSession(sessionId) {
        ...
        try {
            ...
            console.warn('[switchObjectSession] 保存当前会话数据失败:', e);
        }
        ...
        // 4. 清空当前消息数组和字卡库（防止 saveData 在切换 SESSION_ID 后将旧数据写入新会话）
        ...
        // 7. 通知所有功能模块重新加载会话数据
        window.dispatchEvent(new CustomEvent('sessionChanged', { detail: { sessionId: sessionId } }));
    }
    ...
    async function reloadSessionData() {
        try {
            ...
            hideTypingIndicatorUI();
            // 通知所有功能模块重新加载会话数据
            window.dispatchEvent(new CustomEvent('sessionChanged', { detail: { sessionId: window.SESSION_ID } }));
            // 滚动到底部
    ...
    // 添加联系人相关事件绑定（不依赖 DOMContentLoaded，支持 DOM 已加载的情况）
    function bindAddContactEvents() {
        ...
        if (addContactCancelBtn) {
            addContactCancelBtn.onclick = () => window.closeAddContactModal();
        }
        const addContactConfirmBtn = document.getElementById('add-contact-confirm-btn');
        if (addContactConfirmBtn) {
            addContactConfirmBtn.onclick = () => {
                if (typeof window.confirmAddContact === 'function') {
        ...
        if (addContactAvatarPreview && addContactAvatarFile) {
            addContactAvatarPreview.onclick = () => addContactAvatarFile.click();
            addContactAvatarFile.onchange = (e) => {
    ...
    // 立即尝试绑定（如果 DOM 已加载）+ DOMContentLoaded 时再绑定一次
    if (document.readyState === 'loading') {
        document.addEventListener('DOMContentLoaded', bindAddContactEvents);
    } else {
        bindAddContactEvents();
    }
    // 延迟兜底绑定（确保所有脚本加载完成后也能绑定）
    setTimeout(bindAddContactEvents, 500);
    setTimeout(bindAddContactEvents, 2000);

    // 全局事件委托兜底：即使上述绑定全部失败，点击也能响应
    document.addEventListener('click', function(e) {
        // 添加联系人按钮
        if (e.target.closest('#chat-list-add-btn') || e.target.closest('#chat-add-contact-btn')) {
            if (typeof window.openAddContactModal === 'function') {
                window.openAddContactModal();
            }
            return;
        ...
        // 取消按钮
        if (e.target.closest('#add-contact-cancel-btn')) {
            if (typeof window.closeAddContactModal === 'function') {
                window.closeAddContactModal();
            }
            return;
        }
        // 确认添加按钮
        if (e.target.closest('#add-contact-confirm-btn')) {
            if (typeof window.confirmAddContact === 'function') {
                window.confirmAddContact();
            }
            return;
        ...
        // 头像预览点击
        if (e.target.closest('#add-contact-avatar-preview')) {
            const fileInput = document.getElementById('add-contact-avatar-file');
    ...
    window.confirmAddContact = async function() {
        try {
            ...
            try {
                ...
                console.error('创建会话失败，使用降级方案:', e);
                // 降级方案：直接创建新会话
                const newId = Date.now().toString(36) + Math.random().toString(36).slice(2);
                const newSession = { id: newId, name: name, createdAt: Date.now() };
                try {
                    const list = await getLatestSessionList();
                    list.push(newSession);
                    window.sessionList = list;
                ...
                } catch(e2) { console.error('降级创建也失败:', e2); }
                // 手动切换会话
                window.SESSION_ID = newId;
                if (typeof SESSION_ID !== 'undefined') SESSION_ID = newId;
            ...
            try {
                ...
                if (!newSessionId) {
                    console.error('SESSION_ID 未设置，无法保存联系人数据');
                    return;
                }
                const storedSettings = (typeof localforage !== 'undefined')
                    ? ((await localforage.getItem(`${getAppPrefix()}${newSessionId}_chatSettings`)) || {})
                    : {};
                ...
                if (typeof localStorage !== 'undefined') {
                    localStorage.setItem(`${getAppPrefix()}${newSessionId}_partnerName`, name);
                }
            ...
            console.error('添加联系人整体失败:', e);
            alert('添加联系人时出错：' + e.message + '\n请刷新页面后重试');
        }
    };
