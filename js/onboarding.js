...
(function() {
    ...
    function addSkipFallbacks() {
        ...
        // 2. 双击页面右上角区域直接进入
        var skipCount = 0;
        splash.addEventListener('dblclick', function(e) {
            enterSite();
        });
        ...
        setTimeout(function() {
            ...
            skipBtn.style.cssText = 'position:fixed;top:16px;right:16px;z-index:9999999;color:rgba(255,255,255,0.5);font-size:12px;cursor:pointer;padding:8px 12px;border:1px solid rgba(255,255,255,0.15);border-radius:20px;backdrop-filter:blur(10px);-webkit-backdrop-filter:blur(10px);transition:all 0.2s;';
    ...
    // 安全初始化：出错时直接跳过
    function safeInitSplash() {
        try {
            initSplash();
        } catch(e) {
            console.warn('Splash init failed, skipping:', e);
            // 出错直接进入
            var splash = document.getElementById('splash-declaration');
            if (splash) splash.style.display = 'none';
        }
        // 无论成功失败都添加跳过兜底
        try { addSkipFallbacks(); } catch(e) {}
    }

    if (document.readyState === 'loading') {
        document.addEventListener('DOMContentLoaded', safeInitSplash);
    } else {
        safeInitSplash();
    }
})();
...
const tourSteps = [
    ...
    {
        element: '#bulletin-btn',
        title: "📰 今日公告",
        content: "点击这里查看<b>每日公告</b>，对方会给你带来每天的小贴士和心情。<br>",
        position: 'bottom'
    },
    {
        title: "✋ 消息操作提示",
...
async function createNewSession(switchToIt = true) {
    ...
    try {
        ...
        console.warn('[createNewSession] 保存当前会话数据失败:', e);
    }
    ...
    // 安全保存到 localforage（检查是否存在）
    const prefix = (typeof APP_PREFIX !== 'undefined') ? APP_PREFIX : (window.APP_PREFIX || 'CHAT_APP_V3_');
    if (typeof localforage !== 'undefined') {
        ...
        // localforage 不可用时，使用 localStorage 作为降级
        try {
            localStorage.setItem(`${prefix}sessionList`, JSON.stringify(sessionList));
        } catch(e) {
