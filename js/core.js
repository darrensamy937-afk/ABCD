...
autoSendInterval: 5,
        ...
        avatarRejectChance: 30,       // 对方拒绝换头像概率（%）
        partnerInviteAvatarChance: 10, // 对方主动邀请我换头像概率（%）
        partnerAutoAvatarChange: false, // 对方主动自己换头像开关
        // ========== AI 功能设置 ==========
        aiEnabled: false,             // AI 总开关
        aiApiKey: 'sk-a27966f9635c4313bb0096e51d777fae', // API 密钥（DeepSeek）
        aiApiUrl: 'https://api.deepseek.com/v1/chat/completions',
        aiModel: 'deepseek-chat',
        aiPersona: '',                // 人设描述
        aiActionDescription: true,    // 使用动作描写
        aiKeepMemory: true,           // 保留记忆
        aiMemoryLength: 20,           // 保留最近N条消息
        aiAutoMessage: true,          // 主动发消息
        aiAutoRedPacket: false,       // 主动发红包
        aiAutoQuote: false,           // 主动引用收藏用户消息
        aiAutoWithdraw: false,        // 主动撤回
        aiAutoMomentsComment: false,  // 主动评论朋友圈
        aiAutoMomentsPost: false,     // 主动发动态
        aiAutoCall: false,            // 主动打电话
        aiAutoAvatarInvite: false,    // 主动提出换头像邀请
        aiAutoLetter: false,          // 主动写信
        aiAutoCheckin: false,         // 主动查岗
        aiAutoMoyu: false,            // 主动发送摸鱼
        aiAutoSticker: false,         // 主动发表情包
        aiAutoBulletin: false,        // 公告AI主动写
        aiActionInterval: 30,         // AI主动行为检查间隔（分钟）
        soundVolume: 0.15,
        bottomCollapseMode: false,
        emojiMixEnabled: true,
        kaomojiMixEnabled: true,
        partnerStickerSearchEnabled: true,
...
const loadData = async () => {
    try {
        ...
        if (savedMessages && Array.isArray(savedMessages)) {
            ...
            window.messages = messages; // 同步给AI引擎
        }
...
const LIBRARY_CONFIG = {
    reply: {
        ...
        tabs: [
            ...
            { id: 'punctuations', name: '标点符号', mode: 'list' }
        ]
    },
    moyu: {
        title: "摸鱼管理",
...
function manageAutoSendTimer() {
    ...
    // 同步启动 AI 主动行为定时器
    if (typeof window.AIEngine !== 'undefined' && window.AIEngine.manageActionTimer) {
        try { window.AIEngine.manageActionTimer(); } catch(e) {}
    }
}
    ...
    if (typeof window.openMoyuModal === 'function') {
        ...
        const updateUI = () => {
            ...
            const _pillSyncMap = {
                ...
                '#sticker-search-toggle': 'partnerStickerSearchEnabled',
...
function createMessageFragment(msg, prevMsg, nextMsg, lastSenderRef) {
    ...
    const isQuestionnaire = msg.type === 'questionnaire';
    ...
    const isAvatarInvite = msg.type === 'avatar-invite' || (msg.avatarInvite && !msg.text && !msg.image);
    const isFoodCard = msg.type === 'food-card' && msg.foodCard;
    const isHealthReportCard = msg.type === 'health-report-card' && msg.healthReportCard;
    const isLocationCard = msg.type === 'location-card' && msg.locationCard;
    const isDoodleCard = msg.type === 'doodle-card' && msg.doodleCard;
    ...
    if (isRedPacket) {
        ...
        content = window.renderQuestionnaireMessage ? window.renderQuestionnaireMessage(msg) : '<div style="padding:10px;color:#667eea;">问卷消息</div>';
    } else if (isShopCard) {
        content = renderShopMessageCard(msg);
    } else if (isAvatarInvite) {
        content = (window.AvatarSwapApp && typeof window.AvatarSwapApp.renderInviteCardHTML === 'function')
            ? window.AvatarSwapApp.renderInviteCardHTML(msg)
            : '<div style="padding:10px;color:#999;">头像邀请消息</div>';
    } else if (isFoodCard) {
        content = (window.FoodApp && typeof window.FoodApp.renderFoodCardHTML === 'function')
            ? window.FoodApp.renderFoodCardHTML(msg)
            : '<div style="padding:10px;color:#999;">吃什么消息</div>';
    } else if (isHealthReportCard) {
        content = (window.HealthApp && typeof window.HealthApp.renderHealthReportCardHTML === 'function')
            ? window.HealthApp.renderHealthReportCardHTML(msg)
            : '<div style="padding:10px;color:#999;">健康报告</div>';
    } else if (isLocationCard) {
        content = (window.MapDetectApp && typeof window.MapDetectApp.renderLocationCardHTML === 'function')
            ? window.MapDetectApp.renderLocationCardHTML(msg)
            : '<div style="padding:10px;color:#999;">位置消息</div>';
    } else if (isDoodleCard) {
        content = (window.DoodleApp && typeof window.DoodleApp.renderDoodleCardHTML === 'function')
            ? window.DoodleApp.renderDoodleCardHTML(msg)
            : '<div style="padding:10px;color:#999;">涂鸦消息</div>';
    } else if (isLinkCard) {
    ...
    if (isRedPacket || isImageOnly || isQuestionnaire) {
        messageDiv.className = `message message-${msg.sender === 'user' ? 'sent' : 'received'} message-image-bubble-none`;
    } else if (isMediaMessage || isLinkCard || isAvatarInvite || isFoodCard || isHealthReportCard || isLocationCard || isDoodleCard) {
...
const addMessage = (message) => {
    ...
    window.messages = messages; // 同步给AI引擎等外部模块
...
// 暴露 addMessage 到全局，供头像交换等模块调用
if (typeof window !== 'undefined' && typeof window.addMessage === 'undefined') {
    try { window.addMessage = addMessage; } catch(e) {}
}
        ...
        function sendMessage(textOverride = null, type = 'normal') {
            ...
            // 对方免打扰时发消息提示
            if (window.DNDApp && window.DNDApp.isPartnerDNDActive() && type === 'normal') {
                var dndState = window.DNDApp.getPartnerDNDState();
                setTimeout(function () {
                    if (typeof showToast === 'function') {
                        showToast('对方当前处于免打扰时段（' + dndState.startTime + ' ~ ' + dndState.endTime + '），暂时不会回复');
                    }
                }, 500);
            }
            ...
            if (text && text.startsWith('/') && type === 'normal') {
                ...
                if (cmd === '/测试涂鸦' || cmd === '/testdoodle') {
                    DOMElements.messageInput.value = '';
                    DOMElements.messageInput.style.height = '36px';
                    DOMElements.messageInput.style.overflow = 'hidden';
                    if (window.DoodleApp && typeof window.DoodleApp.partnerSendDoodle === 'function') window.DoodleApp.partnerSendDoodle();
                    if (typeof showNotification === 'function') showNotification('✦ 强制触发对方发送涂鸦', 'info', 1800);
                    return;
                }
            }
...
if (!isBatchMode && type === 'normal') {
    // 好友联机会话不触发 AI 自动回复（回复来自好友实时通信）
    var _isFriendSession = (window.FriendConnect && typeof window.FriendConnect.isFriendSession === 'function' && window.FriendConnect.isFriendSession(window.SESSION_ID));
    ...
    window._pendingReplyTimer = null;
            ...
            if (!shouldIgnore && !_isFriendSession) {
        ...
        // 暴露全局 typing indicator 控制函数，供问卷模块等使用
        window._positionTypingIndicator = positionTypingIndicator;
        window._showTypingIndicatorWithText = function(text) {
            try {
                ...
                if (tiLabel) tiLabel.textContent = text || ((settings.partnerName || '对方') + ' 正在输入');
                if (tiWrapper) {
                    positionTypingIndicator();
                    tiWrapper.style.display = 'block';
                }
                if (tiAvatar) {
                    var partnerImg = DOMElements.partner.avatar.querySelector('img');
                    tiAvatar.innerHTML = partnerImg ? '<img src="' + partnerImg.src + '">' : '<i class="fas fa-user"></i>';
                }
                if (DOMElements.chatContainer) DOMElements.chatContainer.scrollTop = DOMElements.chatContainer.scrollHeight;
            } catch(e) { console.warn('[typingIndicator] show error:', e); }
        };
        window._hideTypingIndicatorGlobal = function() {
            try {
...
if (partnerPersonas && partnerPersonas.length > 0 && Math.random() < 0.3) {
            ...
            // ── 对方主动行为：头像邀请/自换、扔水果、切水果邀请、吃什么、位置分享
            // 每个行为独立概率检查，和拍一拍一样在每次回复时触发
            (function() {
                // 对方主动邀请换头像
                if (Math.random() < 0.02) {
                    if (window.AvatarSwapApp && typeof window.AvatarSwapApp.partnerAutoInviteMe === 'function') {
                        window.AvatarSwapApp.partnerAutoInviteMe();
                    }
                }
                // 对方主动自己换头像
                if (settings.partnerAutoAvatarChange && Math.random() < 0.01) {
                    if (window.AvatarSwapApp && typeof window.AvatarSwapApp.partnerAutoChangeOwnAvatar === 'function') {
                        window.AvatarSwapApp.partnerAutoChangeOwnAvatar();
                    }
                }
                // 对方扔水果回来
                if (Math.random() < 0.04) {
                    if (window.FruitApp && typeof window.FruitApp.partnerThrowFruit === 'function') {
                        window.FruitApp.partnerThrowFruit();
                    }
                }
                // 对方邀请切水果
                if (Math.random() < 0.02) {
                    if (window.FruitApp && typeof window.FruitApp.partnerInviteSlice === 'function') {
                        window.FruitApp.partnerInviteSlice();
                    }
                }
                // 对方问吃什么
                if (Math.random() < 0.03) {
                    if (window.FoodApp && typeof window.FoodApp.partnerAskMeFood === 'function') {
                        window.FoodApp.partnerAskMeFood();
                    }
                }
                // 对方分享位置
                if (Math.random() < 0.02) {
                    if (window.MapDetectApp && typeof window.MapDetectApp.partnerRandomSendLocation === 'function') {
                        window.MapDetectApp.partnerRandomSendLocation();
                    }
                }
                // 对方主动发送涂鸦（概率触发，和拍一拍一样在每次回复时独立检查）
                if (Math.random() < 0.02) {
                    if (window.DoodleApp && typeof window.DoodleApp.partnerSendDoodle === 'function') {
                        window.DoodleApp.partnerSendDoodle();
                    }
                }
                // 对方使用标点符号拼字卡（开关开启后概率触发）
                if (Math.random() < 0.03) {
                    if (window.PunctuationLib && window.PunctuationLib.isPunctCardEnabled
                        && typeof window.PunctuationLib.triggerPunctuationCard === 'function'
                        && window.PunctuationLib.isPunctCardEnabled()) {
                        window.PunctuationLib.triggerPunctuationCard();
                    }
                }
            })();
            ...
            if (typeof window.AIEngine !== 'undefined' && window.AIEngine.isAIEnabled && window.AIEngine.isAIEnabled()) {
                showTypingIndicator();
                // 使用用户设定的回复节奏
                var _aiMinDelay = settings.replyDelayMin || 3000;
                var _aiMaxDelay = settings.replyDelayMax || 7000;
                var _aiDelay = _aiMinDelay + Math.random() * (_aiMaxDelay - _aiMinDelay);
                setTimeout(function() {
                    window.AIEngine.generateReply(function(err, aiMsgs) {
                        // 隐藏打字指示器
                        try {
                            (function(){
                        ...
                        if (err || !aiMsgs || aiMsgs.length === 0) {
                            // AI 出错，降级到字卡库
                            return;
                        }

                        // 发送 AI 生成的消息
                        aiMsgs.forEach(function(text, i) {
                            var msgId = Date.now() + i;
                            addMessage({
                                id: msgId,
                            ...
                            if (settings.partnerWithdrawChance > 0 && Math.random() * 100 < settings.partnerWithdrawChance) {
                                var withdrawDelay = 2000 + Math.random() * 5000;
                                setTimeout(function() {
                        ...
                        // AI 模式下也可以发表情包
                        if (settings.aiAutoSticker && typeof window.AIEngine !== 'undefined' && window.AIEngine.getStickerPool) {
                            var stickerPool = window.AIEngine.getStickerPool();
                            if (stickerPool.length > 0 && Math.random() < 0.15) {
                                setTimeout(function() {
                                    window.AIEngine.triggerSticker();
                                }, 800 + Math.random() * 600);
                            }
                        }
                        ...
                        // AI 回复后随机触发主动行为
                        if (typeof window.AIEngine !== 'undefined' && window.AIEngine.maybeActionAfterReply) {
                            try { window.AIEngine.maybeActionAfterReply(); } catch(e) {}
                        }
                    });
                }, _aiDelay);
                return;
