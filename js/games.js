...
function renderFavorites() {
    ...
    list.innerHTML = favoritedMessages.map(msg => {
        ...
        if (msg.type === 'share' && msg.shareData) {
            ...
            const imgSrc = (typeof stickerImg === 'function') ? stickerImg(msg.image) : msg.image;
            content = `<img src="${imgSrc}" style="max-width:100%;max-height:180px;border-radius:8px;display:block;margin-top:4px;cursor:pointer;" onclick="if(typeof viewImage==='function')viewImage('${msg.image.replace(/'/g,'\\\'')}')" loading="lazy" onerror="window._stickerImgError&&window._stickerImgError(this)">`;
        }
...
window._runMsgSearch = function() {
    ...
    resultsEl.innerHTML = results.slice(0, 100).map(msg => {
        ...
        const content = msg.text
            ...
            : (msg.image ? `<img src="${(typeof stickerImg === 'function') ? stickerImg(msg.image) : msg.image}" style="max-height:60px;border-radius:6px;display:block;margin-top:4px;" loading="lazy" onerror="window._stickerImgError&&window._stickerImgError(this)">` : '');
...
function initComboMenu() {
    ...
    function switchTab(tabId) {
        ...
        // 重置contentArea样式（搜狗搜索标签会修改flex布局）
        contentArea.style.display = '';
        contentArea.style.flexDirection = '';
        contentArea.style.gap = '';
        ...
        } else if (tabId === 'sticker-pack') {
            renderStickerPackTab();
        } else if (tabId === 'sogou-search') {
            renderSogouSearchTab();
        } else {
            renderUserPokeMenu();
        }
    }
    ...
    function makeStickerItem(src, onClick) {
        ...
        const proxiedUrl = stickerImg(src);
        item.innerHTML = `<img src="${proxiedUrl}" loading="lazy" onerror="window._stickerImgError&&window._stickerImgError(this)">`;
    ...
    function makeDeletableStickerItem(src, onClick, onDelete) {
        ...
        const proxiedUrl = stickerImg(src);
        item.innerHTML = `<img src="${proxiedUrl}" loading="lazy" onerror="window._stickerImgError&&window._stickerImgError(this)"><div class="sticker-delete-btn" title="删除"><i class="fas fa-times"></i></div>`;
    ...
    // === 搜狗搜索标签页 ===
    var _sogouSearchOwner = 'mine'; // mine / partner
    var _sogouLastResults = [];
    var _sogouPartnerSearchHistory = [];
    var _sogouPartnerStickers = [];

    // 加载对方搜索历史和表情库
    try {
        var savedHistory = localStorage.getItem(getStorageKey('sogouPartnerSearchHistory'));
        if (savedHistory) _sogouPartnerSearchHistory = JSON.parse(savedHistory) || [];
        var savedStickers = localStorage.getItem(getStorageKey('sogouPartnerStickers'));
        if (savedStickers) _sogouPartnerStickers = JSON.parse(savedStickers) || [];
    } catch(e) {}

    function _saveSogouPartnerData() {
        try {
            localStorage.setItem(getStorageKey('sogouPartnerSearchHistory'), JSON.stringify(_sogouPartnerSearchHistory));
            localStorage.setItem(getStorageKey('sogouPartnerStickers'), JSON.stringify(_sogouPartnerStickers));
        } catch(e) {}
    }

    function _addSogouPartnerHistory(kw) {
        if (!kw) return;
        var now = new Date();
        var timeStr = (now.getMonth()+1) + '月' + now.getDate() + '日 ' + String(now.getHours()).padStart(2,'0') + ':' + String(now.getMinutes()).padStart(2,'0');
        _sogouPartnerSearchHistory.push({ keyword: kw, timeStr: timeStr, time: now.getTime() });
        if (_sogouPartnerSearchHistory.length > 50) _sogouPartnerSearchHistory = _sogouPartnerSearchHistory.slice(-50);
        _saveSogouPartnerData();
    }

    function _saveSogouPartnerSticker(url) {
        if (!url) return;
        if (_sogouPartnerStickers.indexOf(url) !== -1) return;
        _sogouPartnerStickers.push(url);
        if (_sogouPartnerStickers.length > 100) _sogouPartnerStickers = _sogouPartnerStickers.slice(-100);
        _saveSogouPartnerData();
    }

    function renderSogouSearchTab() {
        contentArea.innerHTML = '';
        ...
        searchRow.style.cssText = 'display:flex;gap:6px;padding:8px 10px;flex-shrink:0;border-bottom:1px solid var(--border-color);';
        searchRow.innerHTML =
            '<input type="text" id="sogou-search-input" placeholder="输入关键词搜索表情包…" maxlength="10" ' +
            'style="flex:1;min-width:0;height:32px;background:var(--secondary-bg);border:1px solid var(--border-color);border-radius:16px;padding:0 12px;font-size:12px;outline:none;color:var(--text-primary);">' +
            '<button id="sogou-search-btn" style="padding:0 14px;height:32px;background:var(--accent-color);color:#fff;border:none;border-radius:16px;font-size:12px;cursor:pointer;flex-shrink:0;">搜索</button>' +
            '<button id="sogou-saveall-btn" title="保存当前搜索结果到我的表情" style="padding:0 10px;height:32px;background:var(--secondary-bg);color:var(--accent-color);border:1px solid var(--accent-color);border-radius:16px;font-size:11px;cursor:pointer;flex-shrink:0;white-space:nowrap;">保存全部</button>';
        contentArea.appendChild(searchRow);

        // 我的/他的 切换
        var toggleRow = document.createElement('div');
        toggleRow.style.cssText = 'display:flex;gap:0;padding:6px 10px 0;flex-shrink:0;';
        toggleRow.innerHTML =
            '<button class="sogou-owner-tab' + (_sogouSearchOwner === 'mine' ? ' active' : '') + '" data-owner="mine" style="flex:1;padding:5px 0;font-size:11px;cursor:pointer;border:1px solid var(--border-color);border-radius:14px 0 0 14px;border-right:none;' + (_sogouSearchOwner === 'mine' ? 'background:var(--accent-color);color:#fff;border-color:var(--accent-color);' : 'background:var(--secondary-bg);color:var(--text-secondary);') + '">我的</button>' +
            '<button class="sogou-owner-tab' + (_sogouSearchOwner === 'partner' ? ' active' : '') + '" data-owner="partner" style="flex:1;padding:5px 0;font-size:11px;cursor:pointer;border:1px solid var(--border-color);border-radius:0 14px 14px 0;' + (_sogouSearchOwner === 'partner' ? 'background:var(--accent-color);color:#fff;border-color:var(--accent-color);' : 'background:var(--secondary-bg);color:var(--text-secondary);') + '">他的</button>';
        contentArea.appendChild(toggleRow);

        // 内容区
        var resultDiv = document.createElement('div');
        resultDiv.id = 'sogou-search-content';
        resultDiv.style.cssText = 'flex:1;overflow-y:auto;padding:8px 10px;min-height:0;';
        contentArea.appendChild(resultDiv);

        // 初始渲染
        if (_sogouSearchOwner === 'partner') {
            _renderSogouPartnerView(resultDiv);
        } else {
            if (_sogouLastResults.length > 0) {
                _renderSogouSearchResults(resultDiv, _sogouLastResults);
            } else {
                // 显示热门推荐区
                _renderSogouHotRecommendations(resultDiv);
            }
        }

        // 绑定事件
        var searchInput = document.getElementById('sogou-search-input');
        var searchBtn = document.getElementById('sogou-search-btn');
        var saveAllBtn = document.getElementById('sogou-saveall-btn');

        if (searchBtn) {
            searchBtn.onclick = function(e) {
                e.stopPropagation();
                _doSogouSearch(resultDiv);
            };
        }
        if (searchInput) {
            searchInput.onkeypress = function(e) {
                if (e.key === 'Enter') { e.stopPropagation(); _doSogouSearch(resultDiv); }
            };
            searchInput.onclick = function(e) { e.stopPropagation(); };
        }
        if (saveAllBtn) {
            saveAllBtn.onclick = function(e) {
                e.stopPropagation();
                _saveAllSogouResults();
            };
        }

        // owner切换
        toggleRow.querySelectorAll('.sogou-owner-tab').forEach(function(btn) {
            btn.onclick = function(e) {
                e.stopPropagation();
                _sogouSearchOwner = btn.dataset.owner;
                toggleRow.querySelectorAll('.sogou-owner-tab').forEach(function(b) {
                    var isActive = b.dataset.owner === _sogouSearchOwner;
                ...
                if (_sogouSearchOwner === 'partner') {
                    ...
                    if (_sogouLastResults.length > 0) {
                        ...
                        resultDiv.innerHTML = '<div style="text-align:center;padding:20px 10px;font-size:12px;color:var(--text-secondary);font-style:italic;">输入关键词搜索搜狗表情包</div>';
                    }
                }
            };
        });
    ...
    // 热门表情包分类推荐
    var HOT_STICKER_CATEGORIES = [
        { name: '热门', icon: '🔥', kw: '热门表情包', color: '#FF6B6B' },
        { name: '搞笑', icon: '😂', kw: '搞笑表情包', color: '#FFE66D' },
        { name: '可爱', icon: '🐱', kw: '可爱表情包', color: '#FF9FF3' },
        { name: '熊猫头', icon: '🐼', kw: '熊猫头表情包', color: '#54A0FF' },
        { name: '柴犬', icon: '🐕', kw: '柴犬表情包', color: '#FECA57' },
        { name: '恋爱', icon: '❤️', kw: '恋爱表情包', color: '#FF6B81' },
        { name: '难过', icon: '😢', kw: '难过表情包', color: '#778CA3' },
        { name: '生气', icon: '😠', kw: '生气表情包', color: '#EE5253' },
        { name: '打工人', icon: '💼', kw: '打工人表情包', color: '#5F27CD' },
        { name: '干饭', icon: '🍔', kw: '吃货表情包', color: '#FF9F43' },
        { name: '晚安', icon: '😴', kw: '晚安表情包', color: '#4834D4' },
        { name: '摸鱼', icon: '🐟', kw: '摸鱼表情包', color: '#00D2D3' },
    ];

    function _renderSogouHotRecommendations(container) {
        var html = '<div style="padding:10px;">';
        html += '<div style="font-size:12px;font-weight:600;margin-bottom:8px;color:var(--text-primary);">🔥 热门表情分类</div>';
        html += '<div style="display:grid;grid-template-columns:repeat(4, 1fr);gap:8px;">';
        HOT_STICKER_CATEGORIES.forEach(function(cat) {
            html += '<button class="hot-sticker-cat" data-kw="' + cat.kw + '" style="border:none;background:var(--secondary-bg);border-radius:10px;padding:10px 4px;cursor:pointer;display:flex;flex-direction:column;align-items:center;gap:4px;transition:all 0.2s;">';
            html += '<span style="font-size:20px;">' + cat.icon + '</span>';
            html += '<span style="font-size:10px;color:var(--text-secondary);">' + cat.name + '</span>';
            html += '</button>';
        });
        html += '</div>';

        // 快速推荐表情（常用emoji，无网络时也能用）
        html += '<div style="font-size:12px;font-weight:600;margin:14px 0 8px;color:var(--text-primary);">✨ 常用表情</div>';
        var quickEmojis = ['😊','😂','🥰','😭','😡','🤔','👀','💅','😴','🎉','💕','👍','🙏','😎','🥺','💔'];
        html += '<div style="display:grid;grid-template-columns:repeat(8, 1fr);gap:6px;">';
        quickEmojis.forEach(function(emoji) {
            html += '<button class="quick-emoji-btn" data-emoji="' + emoji + '" style="border:none;background:var(--secondary-bg);border-radius:8px;padding:8px 0;cursor:pointer;font-size:18px;transition:all 0.15s;">';
            html += emoji;
            html += '</button>';
        });
        html += '</div>';
        ...
        // 绑定分类点击
        container.querySelectorAll('.hot-sticker-cat').forEach(function(btn) {
            btn.onclick = function(e) {
                e.stopPropagation();
                var kw = btn.dataset.kw;
                var input = document.getElementById('sogou-search-input');
                if (input) input.value = kw;
                _doSogouSearch(container);
            };
        });

        // 绑定常用表情点击（直接发送）
        container.querySelectorAll('.quick-emoji-btn').forEach(function(btn) {
            btn.onclick = function(e) {
                e.stopPropagation();
                var emoji = btn.dataset.emoji;
                addMessage({ id: Date.now(), sender: 'user', text: emoji, timestamp: new Date(), status: 'sent', type: 'normal' });
                playSound('send');
                picker.classList.remove('active');
    ...
    function _doSogouSearch(container) {
        if (_sogouSearchOwner === 'partner') {
            showNotification('切换到"我的"才能搜索', 'info', 1500);
            return;
        }
        var input = document.getElementById('sogou-search-input');
        if (!input) return;
        var kw = input.value.trim();
        if (!kw) { showNotification('请输入关键词', 'info', 1200); return; }
        container.innerHTML = '<div style="text-align:center;padding:20px;font-size:12px;color:var(--text-secondary);">搜索中…</div>';

        var searchFn = window.searchSogouStickers;
        if (typeof searchFn !== 'function') {
            container.innerHTML = '<div style="text-align:center;padding:16px 10px;font-size:11px;color:var(--text-secondary);line-height:1.6;">搜索功能加载中，请稍后重试</div>';
            return;
        }
        searchFn(kw).then(function(results) {
            _sogouLastResults = results || [];
            _renderSogouSearchResults(container, _sogouLastResults);
        }).catch(function() {
            container.innerHTML = '<div style="text-align:center;padding:16px 10px;font-size:11px;color:#e74c3c;line-height:1.6;">搜索失败，请稍后重试</div>';
        });
    }

    function _renderSogouSearchResults(container, urls) {
        container.innerHTML = '';
        if (!urls || urls.length === 0) {
            container.innerHTML = '<div style="text-align:center;padding:16px 10px;font-size:11px;color:var(--text-secondary);line-height:1.6;">未找到相关表情包<br>换个关键词试试吧</div>';
            return;
        }
        var grid = document.createElement('div');
        grid.className = 'sticker-grid-view';
        urls.forEach(function(url) {
            var saved = myStickerLibrary && myStickerLibrary.indexOf(url) !== -1;
            var item = document.createElement('div');
            item.className = 'sticker-grid-item';
            item.style.cssText = 'position:relative;width:56px;height:56px;border-radius:8px;overflow:hidden;cursor:pointer;background:var(--secondary-bg);flex-shrink:0;';
            item.innerHTML = '<img src="' + stickerImg(url) + '" loading="lazy" style="width:100%;height:100%;object-fit:cover;" onerror="window._stickerImgError&&window._stickerImgError(this)">' +
                '<div class="sogou-save-btn' + (saved ? ' saved' : '') + '" title="' + (saved ? '已保存' : '点击保存') + '" style="position:absolute;top:2px;right:2px;width:16px;height:16px;border-radius:50%;background:' + (saved ? 'var(--accent-color)' : 'rgba(0,0,0,0.5)') + ';color:#fff;font-size:10px;display:flex;align-items:center;justify-content:center;">' + (saved ? '✓' : '+') + '</div>';
            // 点击图片发送
            item.querySelector('img').onclick = function(e) {
                e.stopPropagation();
                addMessage({ id: Date.now(), sender: 'user', text: '', timestamp: new Date(), image: url, status: 'sent', type: 'normal' });
                playSound('send');
                picker.classList.remove('active');
                var delayRange = settings.replyDelayMax - settings.replyDelayMin;
                setTimeout(simulateReply, settings.replyDelayMin + Math.random() * delayRange);
            ...
            // 点击保存按钮
            var saveBtn = item.querySelector('.sogou-save-btn');
            saveBtn.onclick = function(e) {
                e.stopPropagation();
                if (saveBtn.classList.contains('saved')) {
                    // 已保存 → 从我的表情库删除
                    var idx = myStickerLibrary.indexOf(url);
                    if (idx !== -1) myStickerLibrary.splice(idx, 1);
                    localforage.setItem(getStorageKey('myStickerLibrary'), myStickerLibrary);
                    saveBtn.classList.remove('saved');
                    saveBtn.style.background = 'rgba(0,0,0,0.5)';
                    saveBtn.textContent = '+';
                    saveBtn.title = '点击保存';
                    showNotification('已从我的表情删除', 'success', 1200);
                } else {
                    // 保存到我的表情库
                    if (!myStickerLibrary) myStickerLibrary = [];
                    myStickerLibrary.push(url);
                    localforage.setItem(getStorageKey('myStickerLibrary'), myStickerLibrary);
                    saveBtn.classList.add('saved');
                    saveBtn.style.background = 'var(--accent-color)';
                    saveBtn.textContent = '✓';
                    saveBtn.title = '已保存';
                    showNotification('已保存到我的表情', 'success', 1200);
                }
            };
            grid.appendChild(item);
        });
    ...
    function _saveAllSogouResults() {
        if (_sogouLastResults.length === 0) {
            showNotification('没有可保存的表情包', 'info', 1200);
            return;
        }
        var count = 0;
        _sogouLastResults.forEach(function(url) {
            if (!myStickerLibrary) myStickerLibrary = [];
            if (myStickerLibrary.indexOf(url) === -1) {
                myStickerLibrary.push(url);
                count++;
            }
        });
        localforage.setItem(getStorageKey('myStickerLibrary'), myStickerLibrary);
        // 更新保存按钮状态
        var container = document.getElementById('sogou-search-content');
        if (container) {
            container.querySelectorAll('.sogou-save-btn').forEach(function(btn) {
        ...
        showNotification('已保存 ' + count + ' 个表情包到我的表情', 'success', 1500);
    }

    function _renderSogouPartnerView(container) {
        container.innerHTML = '';
        // 对方搜索记录
        var historySection = document.createElement('div');
        historySection.style.cssText = 'margin-bottom:8px;';
        historySection.innerHTML = '<div style="font-size:10px;color:var(--text-secondary);margin-bottom:4px;">搜索记录 (' + _sogouPartnerSearchHistory.length + ')</div>';
        if (_sogouPartnerSearchHistory.length === 0) {
            historySection.innerHTML += '<div style="text-align:center;padding:20px 10px;font-size:11px;color:var(--text-secondary);font-style:italic;">对方还没有搜索记录</div>';
        } else {
            _sogouPartnerSearchHistory.slice().reverse().forEach(function(h) {
                var item = document.createElement('div');
                item.style.cssText = 'display:flex;align-items:center;justify-content:space-between;padding:6px 10px;border-bottom:1px solid var(--border-color);font-size:11px;cursor:pointer;';
                item.innerHTML = '<span style="color:var(--text-primary);font-weight:500;">🔍 ' + h.keyword + '</span><span style="color:var(--text-secondary);font-size:10px;">' + (h.timeStr || '') + '</span>';
                item.onclick = function(e) {
                    e.stopPropagation();
                    // 切换到"我的"并搜索
                    _sogouSearchOwner = 'mine';
                    var ownerTabs = contentArea.querySelectorAll('.sogou-owner-tab');
                    ownerTabs.forEach(function(b) {
                        var isActive = b.dataset.owner === 'mine';
                        b.classList.toggle('active', isActive);
                        if (isActive) {
                            b.style.background = 'var(--accent-color)';
                            b.style.color = '#fff';
                    ...
                    var input = document.getElementById('sogou-search-input');
                    if (input) input.value = h.keyword;
                    _doSogouSearch(container);
                };
        ...
        // 对方表情库
        var stickerSection = document.createElement('div');
        stickerSection.innerHTML = '<div style="font-size:10px;color:var(--text-secondary);margin-bottom:4px;">他的表情库 (' + _sogouPartnerStickers.length + ')</div>';
        if (_sogouPartnerStickers.length === 0) {
            stickerSection.innerHTML += '<div style="text-align:center;padding:20px 10px;font-size:11px;color:var(--text-secondary);font-style:italic;">对方还没有保存表情包</div>';
        } else {
            var grid = document.createElement('div');
            grid.className = 'sticker-grid-view';
            _sogouPartnerStickers.forEach(function(url) {
                var item = document.createElement('div');
                item.className = 'sticker-grid-item';
                item.style.cssText = 'width:56px;height:56px;border-radius:8px;overflow:hidden;cursor:pointer;background:var(--secondary-bg);flex-shrink:0;';
                item.innerHTML = '<img src="' + stickerImg(url) + '" loading="lazy" style="width:100%;height:100%;object-fit:cover;" onerror="window._stickerImgError&&window._stickerImgError(this)">';
                item.onclick = function(e) {
                    e.stopPropagation();
                    // 点击发送对方保存的表情
            ...
            stickerSection.appendChild(grid);
        }
        container.appendChild(stickerSection);
    }

    // === 网络表情包标签页 ===
    var _stickerPackCache = {}; // packId -> items[]
    var _stickerPackLoading = {}; // packId -> bool

    // 表情包图片URL处理：本地缓存URL直接使用，外网URL走代理
    function stickerImg(url){
      ...
      return '/api/sticker-img?url=' + encodeURIComponent(url);
    }
    // 全局暴露，供消息渲染等外部调用
    window.stickerImg = stickerImg;

    // 表情包图片加载失败处理：隐藏坏图，保持网格整洁
    ...
    function renderStickerPackTab() {
        ...
        targetRow.style.cssText = 'display:flex;gap:0;padding:6px 10px;flex-shrink:0;border-bottom:1px solid var(--border-color);';
        targetRow.innerHTML =
            '<span style="font-size:11px;color:var(--text-secondary);line-height:28px;margin-right:8px;">添加到：</span>' +
            '<button class="pack-target-btn active" data-target="mine" style="flex:0 0 auto;padding:4px 12px;font-size:11px;cursor:pointer;border:1px solid var(--accent-color);border-radius:14px;background:var(--accent-color);color:#fff;margin-right:4px;">我的</button>' +
            '<button class="pack-target-btn" data-target="partner" style="flex:0 0 auto;padding:4px 12px;font-size:11px;cursor:pointer;border:1px solid var(--border-color);border-radius:14px;background:var(--secondary-bg);color:var(--text-secondary);">对方</button>';
        contentArea.appendChild(targetRow);

        var currentTarget = 'mine';
        targetRow.querySelectorAll('.pack-target-btn').forEach(function(btn) {
            btn.onclick = function(e) {
                e.stopPropagation();
                currentTarget = btn.dataset.target;
                targetRow.querySelectorAll('.pack-target-btn').forEach(function(b) {
                    if (b.dataset.target === currentTarget) {
        ...
        // 表情包网格列表
        var packListDiv = document.createElement('div');
        packListDiv.id = 'sticker-pack-list';
        packListDiv.style.cssText = 'flex:1;overflow-y:auto;padding:8px 10px;min-height:0;';
        packListDiv.innerHTML = '<div style="text-align:center;padding:20px;font-size:12px;color:var(--text-secondary);">加载表情包列表…</div>';
        contentArea.appendChild(packListDiv);

        // 表情包详情区（点击某个表情包后显示）
        var packDetailDiv = document.createElement('div');
        packDetailDiv.id = 'sticker-pack-detail';
        packDetailDiv.style.cssText = 'display:none;flex:1;overflow-y:auto;padding:8px 10px;min-height:0;';
        contentArea.appendChild(packDetailDiv);

        // 加载表情包列表
        loadStickerPackList(packListDiv, packDetailDiv, function() { return currentTarget; });
    }

    function loadStickerPackList(listContainer, detailContainer, getTarget) {
        fetch('/api/sticker-packs').then(function(r) { return r.json(); }).then(function(data) {
            if (data.code !== 200 || !data.packs) {
                listContainer.innerHTML = '<div style="text-align:center;padding:20px;font-size:12px;color:var(--text-secondary);">加载失败，请刷新重试</div>';
                return;
            }
            var grid = document.createElement('div');
            ...
            data.packs.forEach(function(pack) {
                var item = document.createElement('div');
                item.style.cssText = 'display:flex;flex-direction:column;align-items:center;padding:12px 4px;border-radius:10px;cursor:pointer;background:var(--secondary-bg);transition:all 0.2s;border:1px solid transparent;';
                item.innerHTML =
                    '<div style="font-size:28px;margin-bottom:4px;">' + pack.icon + '</div>' +
                    '<div style="font-size:11px;color:var(--text-primary);text-align:center;">' + pack.name + '</div>';
                ...
                item.onclick = function(e) {
                    e.stopPropagation();
                    openStickerPackDetail(pack, listContainer, detailContainer, getTarget);
                };
                grid.appendChild(item);
            });
            ...
            listContainer.innerHTML = '<div style="text-align:center;padding:20px;font-size:12px;color:var(--text-secondary);">网络错误，请稍后重试</div>';
        });
    }

    function openStickerPackDetail(pack, listContainer, detailContainer, getTarget) {
        ...
        header.style.cssText = 'display:flex;align-items:center;gap:8px;padding:0 0 8px;border-bottom:1px solid var(--border-color);margin-bottom:8px;';
        header.innerHTML =
            '<button id="pack-back-btn" style="background:none;border:none;cursor:pointer;color:var(--accent-color);font-size:14px;padding:4px;"><i class="fas fa-arrow-left"></i></button>' +
            '<span style="font-size:14px;">' + pack.icon + ' ' + pack.name + '</span>' +
            '<button id="pack-addall-btn" style="margin-left:auto;background:var(--accent-color);color:#fff;border:none;border-radius:12px;padding:4px 12px;font-size:11px;cursor:pointer;white-space:nowrap;">全部添加</button>';
        detailContainer.appendChild(header);

        document.getElementById('pack-back-btn').onclick = function(e) {
            e.stopPropagation();
        ...
        loadingDiv.style.cssText = 'text-align:center;padding:20px;font-size:12px;color:var(--text-secondary);';
        loadingDiv.textContent = '加载表情包内容…';
        detailContainer.appendChild(loadingDiv);

        // 检查缓存
        if (_stickerPackCache[pack.id]) {
            renderStickerPackItems(detailContainer, loadingDiv, _stickerPackCache[pack.id], pack, getTarget);
            return;
        }

        // 请求API（POST方式，避免中文编码问题）
        fetch('/api/sticker-pack-items', {
            ...
            _stickerPackCache[pack.id] = items;
            renderStickerPackItems(detailContainer, loadingDiv, items, pack, getTarget);
        }).catch(function() {
            loadingDiv.innerHTML = '加载失败，请返回重试';
        });
    }

    function renderStickerPackItems(detailContainer, loadingDiv, items, pack, getTarget) {
        loadingDiv.remove();
        if (items.length === 0) {
            detailContainer.innerHTML += '<div style="text-align:center;padding:20px;font-size:12px;color:var(--text-secondary);">该表情包暂无内容</div>';
            return;
        }
        var grid = document.createElement('div');
        grid.className = 'sticker-grid-view';
        items.forEach(function(url) {
            ...
            item.style.cssText = 'position:relative;width:56px;height:56px;border-radius:8px;overflow:hidden;cursor:pointer;background:var(--secondary-bg);flex-shrink:0;';
            item.innerHTML = '<img src="' + stickerImg(url) + '" loading="lazy" style="width:100%;height:100%;object-fit:cover;" onerror="window._stickerImgError&&window._stickerImgError(this)">' +
                '<div class="pack-add-btn" title="添加" style="position:absolute;top:2px;right:2px;width:16px;height:16px;border-radius:50%;background:rgba(0,0,0,0.5);color:#fff;font-size:10px;display:flex;align-items:center;justify-content:center;">+</div>';
            // 点击图片发送
            item.querySelector('img').onclick = function(e) {
                e.stopPropagation();
                addMessage({ id: Date.now(), sender: 'user', text: '', timestamp: new Date(), image: url, status: 'sent', type: 'normal' });
            ...
            // 点击添加按钮
            var addBtn = item.querySelector('.pack-add-btn');
            addBtn.onclick = function(e) {
                e.stopPropagation();
                var target = getTarget();
                addStickerPackItem(url, target, addBtn);
            };
            grid.appendChild(item);
        });
        ...
        // 全部添加按钮
        var addAllBtn = document.getElementById('pack-addall-btn');
        if (addAllBtn) {
            addAllBtn.onclick = function(e) {
                ...
                var count = 0;
                items.forEach(function(url) {
                    if (addStickerPackItemSilent(url, target)) count++;
                });
                // 更新所有按钮状态
                detailContainer.querySelectorAll('.pack-add-btn').forEach(function(btn) {
                    btn.classList.add('saved');
                    btn.style.background = 'var(--accent-color)';
                    btn.textContent = '✓';
                });
                showNotification('已添加 ' + count + ' 个表情到' + (target === 'mine' ? '我的表情库' : '对方表情库'), 'success', 1500);
            };
        }
    }

    function addStickerPackItemSilent(url, target) {
        if (target === 'mine') {
            if (!myStickerLibrary) myStickerLibrary = [];
            if (myStickerLibrary.indexOf(url) !== -1) return false;
            myStickerLibrary.push(url);
            localforage.setItem(getStorageKey('myStickerLibrary'), myStickerLibrary);
            return true;
        } else {
            if (_sogouPartnerStickers.indexOf(url) !== -1) return false;
            _sogouPartnerStickers.push(url);
            _saveSogouPartnerData();
            return true;
        }
    ...
    function addStickerPackItem(url, target, btn) {
        var added = addStickerPackItemSilent(url, target);
        if (added) {
            btn.classList.add('saved');
            btn.style.background = 'var(--accent-color)';
            btn.textContent = '✓';
            showNotification('已添加到' + (target === 'mine' ? '我的表情库' : '对方表情库'), 'success', 1200);
        } else {
            // 已存在则删除
            if (target === 'mine') {
                var idx = myStickerLibrary.indexOf(url);
                ...
                var idx2 = _sogouPartnerStickers.indexOf(url);
                if (idx2 !== -1) _sogouPartnerStickers.splice(idx2, 1);
                _saveSogouPartnerData();
            }
            btn.classList.remove('saved');
            btn.style.background = 'rgba(0,0,0,0.5)';
            btn.textContent = '+';
            showNotification('已移除', 'info', 1000);
        }
    }

    // 暴露给对方自动搜索使用
    window._sogouPartnerStickerAPI = {
        addHistory: _addSogouPartnerHistory,
        saveSticker: _saveSogouPartnerSticker,
        getHistory: function() { return _sogouPartnerSearchHistory; },
        getStickers: function() { return _sogouPartnerStickers; }
    };
}
