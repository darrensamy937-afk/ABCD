    ...
    setupAvatarFrameSettings();
            ...
            const settingToggles = {
                ...
                '#typing-indicator-toggle': {
                    ...
                    '#sticker-search-toggle': { prop: 'partnerStickerSearchEnabled', name: '对方主动搜索表情包' },
                    '#enter-key-send-toggle': { prop: 'enterKeySendEnabled', name: '回车键发送' }
};
            ...
            const _extraToggleSync = {
                ...
                '#partner-auto-avatar-change-toggle': 'partnerAutoAvatarChange',
                // AI 功能开关
                '#ai-enabled-toggle': 'aiEnabled',
                '#ai-action-description-toggle': 'aiActionDescription',
                '#ai-keep-memory-toggle': 'aiKeepMemory',
                '#ai-auto-message-toggle': 'aiAutoMessage',
                '#ai-auto-red-packet-toggle': 'aiAutoRedPacket',
                '#ai-auto-quote-toggle': 'aiAutoQuote',
                '#ai-auto-withdraw-toggle': 'aiAutoWithdraw',
                '#ai-auto-moments-comment-toggle': 'aiAutoMomentsComment',
                '#ai-auto-moments-post-toggle': 'aiAutoMomentsPost',
                '#ai-auto-call-toggle': 'aiAutoCall',
                '#ai-auto-avatar-invite-toggle': 'aiAutoAvatarInvite',
                '#ai-auto-letter-toggle': 'aiAutoLetter',
                '#ai-auto-checkin-toggle': 'aiAutoCheckin',
                '#ai-auto-moyu-toggle': 'aiAutoMoyu',
                '#ai-auto-sticker-toggle': 'aiAutoSticker',
                '#ai-auto-bulletin-toggle': 'aiAutoBulletin'
            };
            ...
            for (const [selector, {
                ...
                const _initVal = (prop === 'emojiMixEnabled' || prop === 'kaomojiMixEnabled' || prop === 'partnerStickerSearchEnabled') ? (settings[prop] !== false) : !!settings[prop];
                element.classList.toggle('active', _initVal);

                element.addEventListener('click', () => {
                    if ((prop === 'emojiMixEnabled' || prop === 'kaomojiMixEnabled' || prop === 'partnerStickerSearchEnabled') && settings[prop] === undefined) settings[prop] = true;
            ...
            // 标点符号拼字卡开关
            const punctCardToggle = document.getElementById('punct-card-toggle');
            if (punctCardToggle) {
                const punctEnabled = window.PunctuationLib && window.PunctuationLib.isPunctCardEnabled
                    ? window.PunctuationLib.isPunctCardEnabled()
                    : false;
                punctCardToggle.classList.toggle('active', punctEnabled);
                punctCardToggle.addEventListener('click', () => {
                    if (window.PunctuationLib && typeof window.PunctuationLib.setPunctCardEnabled === 'function') {
                        const newVal = !window.PunctuationLib.isPunctCardEnabled();
                        window.PunctuationLib.setPunctCardEnabled(newVal);
                        punctCardToggle.classList.toggle('active', newVal);
                        showNotification('标点符号拼字卡已' + (newVal ? '开启' : '关闭'), 'success');
                    }
                });
            }
...
// 对方拒绝换头像概率
const avatarRejectSlider = document.getElementById('avatar-reject-chance-slider');
const avatarRejectDisplay = document.getElementById('avatar-reject-chance-value');
if (avatarRejectSlider) {
    avatarRejectSlider.value = settings.avatarRejectChance !== undefined ? settings.avatarRejectChance : 30;
    if (avatarRejectDisplay) avatarRejectDisplay.textContent = (settings.avatarRejectChance !== undefined ? settings.avatarRejectChance : 30) + '%';
    avatarRejectSlider.addEventListener('input', function() {
        settings.avatarRejectChance = parseInt(this.value);
        if (avatarRejectDisplay) avatarRejectDisplay.textContent = this.value + '%';
    });
    avatarRejectSlider.addEventListener('change', function() {
        throttledSaveData();
    });
}

// 对方主动邀我换头像概率
const partnerInviteAvatarSlider = document.getElementById('partner-invite-avatar-chance-slider');
const partnerInviteAvatarDisplay = document.getElementById('partner-invite-avatar-chance-value');
if (partnerInviteAvatarSlider) {
    partnerInviteAvatarSlider.value = settings.partnerInviteAvatarChance !== undefined ? settings.partnerInviteAvatarChance : 10;
    if (partnerInviteAvatarDisplay) partnerInviteAvatarDisplay.textContent = (settings.partnerInviteAvatarChance !== undefined ? settings.partnerInviteAvatarChance : 10) + '%';
    partnerInviteAvatarSlider.addEventListener('input', function() {
        settings.partnerInviteAvatarChance = parseInt(this.value);
        if (partnerInviteAvatarDisplay) partnerInviteAvatarDisplay.textContent = this.value + '%';
    });
    partnerInviteAvatarSlider.addEventListener('change', function() {
        throttledSaveData();
    });
}
...
// ========== 扔水果 & 切水果概率设置 ==========
// 对方主动扔水果概率
const partnerThrowChanceSlider = document.getElementById('partner-throw-chance-slider');
const partnerThrowChanceDisplay = document.getElementById('partner-throw-chance-value');
if (partnerThrowChanceSlider) {
    var _initThrow = 15;
    if (window.FruitApp && window.FruitApp.getSettings) {
        _initThrow = Math.round((window.FruitApp.getSettings().partnerThrowChance || 0.15) * 100);
    }
    partnerThrowChanceSlider.value = _initThrow;
    if (partnerThrowChanceDisplay) partnerThrowChanceDisplay.textContent = _initThrow + '%';
    partnerThrowChanceSlider.addEventListener('input', function() {
        if (partnerThrowChanceDisplay) partnerThrowChanceDisplay.textContent = this.value + '%';
    });
    partnerThrowChanceSlider.addEventListener('change', function() {
        if (window.FruitApp && window.FruitApp.setPartnerThrowChance) {
            window.FruitApp.setPartnerThrowChance(parseInt(this.value) / 100);
        }
    });
}
// 对方主动邀我切水果概率
const partnerSliceInviteSlider = document.getElementById('partner-slice-invite-chance-slider');
const partnerSliceInviteDisplay = document.getElementById('partner-slice-invite-chance-value');
if (partnerSliceInviteSlider) {
    var _initSlice = 10;
    if (window.FruitApp && window.FruitApp.getSettings) {
        _initSlice = Math.round((window.FruitApp.getSettings().partnerSliceInviteChance || 0.10) * 100);
    }
    partnerSliceInviteSlider.value = _initSlice;
    if (partnerSliceInviteDisplay) partnerSliceInviteDisplay.textContent = _initSlice + '%';
    partnerSliceInviteSlider.addEventListener('input', function() {
        if (partnerSliceInviteDisplay) partnerSliceInviteDisplay.textContent = this.value + '%';
    });
    partnerSliceInviteSlider.addEventListener('change', function() {
        if (window.FruitApp && window.FruitApp.setPartnerSliceInviteChance) {
            window.FruitApp.setPartnerSliceInviteChance(parseInt(this.value) / 100);
        }
    });
}
// 对方扔水果后随机回复概率
const partnerFruitReplySlider = document.getElementById('partner-fruit-reply-chance-slider');
const partnerFruitReplyDisplay = document.getElementById('partner-fruit-reply-chance-value');
if (partnerFruitReplySlider) {
    var _initReply = 100;
    if (window.FruitApp && window.FruitApp.getSettings) {
        _initReply = Math.round((window.FruitApp.getSettings().partnerReplyChance !== undefined ? window.FruitApp.getSettings().partnerReplyChance : 1) * 100);
    }
    partnerFruitReplySlider.value = _initReply;
    if (partnerFruitReplyDisplay) partnerFruitReplyDisplay.textContent = _initReply + '%';
    partnerFruitReplySlider.addEventListener('input', function() {
        if (partnerFruitReplyDisplay) partnerFruitReplyDisplay.textContent = this.value + '%';
    });
    partnerFruitReplySlider.addEventListener('change', function() {
        if (window.FruitApp && window.FruitApp.setPartnerReplyChance) {
            window.FruitApp.setPartnerReplyChance(parseInt(this.value) / 100);
        }
    });
}

// 对方主动问我吃什么概率（已移除概率设置，改为纯随机触发，此滑块仅做UI展示）
const partnerFoodAskSlider = document.getElementById('partner-food-ask-chance-slider');
const partnerFoodAskDisplay = document.getElementById('partner-food-ask-chance-value');
if (partnerFoodAskSlider) {
    partnerFoodAskSlider.value = 50;
    if (partnerFoodAskDisplay) partnerFoodAskDisplay.textContent = '50%';
    partnerFoodAskSlider.addEventListener('input', function() {
        if (partnerFoodAskDisplay) partnerFoodAskDisplay.textContent = this.value + '%';
    });
    partnerFoodAskSlider.addEventListener('change', function() {
        // 纯随机触发，无需保存概率值
    });
}

// 对方主动自己换头像开关
const partnerAutoAvatarChangeToggle = document.getElementById('partner-auto-avatar-change-toggle');
if (partnerAutoAvatarChangeToggle) {
    partnerAutoAvatarChangeToggle.addEventListener('click', function() {
        settings.partnerAutoAvatarChange = !settings.partnerAutoAvatarChange;
        this.classList.toggle('active', !!settings.partnerAutoAvatarChange);
        throttledSaveData();
        showNotification('对方主动自己换头像已' + (settings.partnerAutoAvatarChange ? '开启' : '关闭'), 'success');
    });
}
...
if (aiEnabledToggle) {
    aiEnabledToggle.addEventListener('click', function() {
        settings.aiEnabled = !settings.aiEnabled;
        this.classList.toggle('active', !!settings.aiEnabled);
        throttledSaveData();
        if (settings.aiEnabled && typeof window.AIEngine !== 'undefined') {
            window.AIEngine.manageActionTimer();
        }
        showNotification('AI功能已' + (settings.aiEnabled ? '开启' : '关闭') + (settings.aiEnabled ? '，对方回复将由AI生成' : '，对方回复恢复字卡库'), 'success');
    });
}

// AI 开关通用处理函数
function setupAIToggle(id, prop, label) {
    var el = document.getElementById(id);
    if (el) {
        el.addEventListener('click', function() {
            settings[prop] = !settings[prop];
            this.classList.toggle('active', !!settings[prop]);
            throttledSaveData();
            showNotification(label + '已' + (settings[prop] ? '开启' : '关闭'), 'success');
        });
    }
}
setupAIToggle('ai-action-description-toggle', 'aiActionDescription', '动作描写');
setupAIToggle('ai-keep-memory-toggle', 'aiKeepMemory', '保留记忆');
setupAIToggle('ai-auto-message-toggle', 'aiAutoMessage', 'AI主动发消息');
setupAIToggle('ai-auto-red-packet-toggle', 'aiAutoRedPacket', 'AI主动发红包');
setupAIToggle('ai-auto-quote-toggle', 'aiAutoQuote', 'AI引用收藏消息');
setupAIToggle('ai-auto-withdraw-toggle', 'aiAutoWithdraw', 'AI主动撤回');
setupAIToggle('ai-auto-moments-comment-toggle', 'aiAutoMomentsComment', 'AI评论朋友圈');
setupAIToggle('ai-auto-moments-post-toggle', 'aiAutoMomentsPost', 'AI发动态');
setupAIToggle('ai-auto-call-toggle', 'aiAutoCall', 'AI主动打电话');
setupAIToggle('ai-auto-avatar-invite-toggle', 'aiAutoAvatarInvite', 'AI换头像邀请');
setupAIToggle('ai-auto-letter-toggle', 'aiAutoLetter', 'AI主动写信');
setupAIToggle('ai-auto-checkin-toggle', 'aiAutoCheckin', 'AI主动查岗');
setupAIToggle('ai-auto-moyu-toggle', 'aiAutoMoyu', 'AI发送摸鱼');
setupAIToggle('ai-auto-sticker-toggle', 'aiAutoSticker', 'AI发表情包');
setupAIToggle('ai-auto-bulletin-toggle', 'aiAutoBulletin', 'AI公告写作');
...
if (aiModelInput) {
    aiModelInput.value = settings.aiModel || 'deepseek-chat';
    aiModelInput.addEventListener('input', function() {
        settings.aiModel = this.value;
    });
...
if (aiPersonaInput) {
    ...
    aiPersonaInput.addEventListener('input', function() {
        settings.aiPersona = this.value;
    });
    aiPersonaInput.addEventListener('change', function() {
        throttledSaveData();
    });
}

// 记忆条数滑块
const aiMemorySlider = document.getElementById('ai-memory-length-slider');
const aiMemoryDisplay = document.getElementById('ai-memory-length-value');
if (aiMemorySlider) {
    aiMemorySlider.value = settings.aiMemoryLength !== undefined ? settings.aiMemoryLength : 20;
    if (aiMemoryDisplay) aiMemoryDisplay.textContent = settings.aiMemoryLength !== undefined ? settings.aiMemoryLength : 20;
    aiMemorySlider.addEventListener('input', function() {
        settings.aiMemoryLength = parseInt(this.value);
        if (aiMemoryDisplay) aiMemoryDisplay.textContent = this.value;
    });
    aiMemorySlider.addEventListener('change', function() {
        throttledSaveData();
    });
}
...
if (moyuReportMinInterval) {
    ...
    moyuReportMinInterval.addEventListener('change', () => throttledSaveData());
}
...
if (moyuReportMaxInterval) {
    ...
    moyuReportMaxInterval.addEventListener('change', () => throttledSaveData());
}
