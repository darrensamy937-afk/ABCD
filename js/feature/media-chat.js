...
(function () {
    ...
    window.toggleChatAttachMenu = function(event) {
        ...
        // 按钮配置：{ icon, label, action } — 底色统一纯白
        const buttons = [
            { icon: 'fa-image', label: '图片', action: 'file' },
            { icon: 'fa-file-audio', label: '音视频', action: 'media-upload' },
            { icon: 'fa-video', label: '视频通话', action: 'call' },
            { icon: 'fa-link', label: '链接', action: 'link' },
            { icon: 'fa-clipboard-list', label: '问卷', action: 'questionnaire' },
            { icon: 'fa-shield-alt', label: '查岗', action: 'checkin' },
            { icon: 'fa-utensils', label: '吃什么', action: 'food' },
            { icon: 'fa-map-marker-alt', label: '位置', action: 'location' },
            { icon: 'fa-search', label: '搜索', action: 'search' },
            { icon: 'fa-apple-alt', label: '扔水果', action: 'fruit-throw' },
            { icon: 'fa-cut', label: '切水果', action: 'fruit-slice' },
            { icon: 'fa-user-circle', label: '换头像', action: 'avatar-swap' },
            { icon: 'fa-paint-brush', label: '涂鸦', action: 'draw' },
            { icon: 'fa-heart', label: '陪伴', action: 'companion' },
            { icon: 'fa-music', label: '音乐分享', action: 'music' },
            { icon: 'fa-bell-slash', label: '免打扰', action: 'dnd' },
            { icon: 'fa-map-marked-alt', label: '地图', action: 'map-detect' },
            { icon: 'fa-heart-pulse', label: '健康检测', action: 'health-detect' },
            { icon: 'fa-cat', label: '桌宠', action: 'desk-pet' },
        ];

        const perPage = 8; // 4列 x 2行
        const totalPages = Math.ceil(buttons.length / perPage);

        let pagesHtml = '';
        for (let p = 0; p < totalPages; p++) {
            const pageBtns = buttons.slice(p * perPage, (p + 1) * perPage);
            let btnsHtml = '';
            pageBtns.forEach(b => {
                btnsHtml += `
                    <button type="button" class="attach-grid-btn" data-action="${b.action}">
                        <div class="attach-grid-icon attach-grid-icon-white">
                            <i class="fas ${b.icon}"></i>
                        </div>
                        <span class="attach-grid-label">${b.label}</span>
                    </button>`;
            });
            pagesHtml += `<div class="attach-page">${btnsHtml}</div>`;
        }

        let dotsHtml = '';
        for (let i = 0; i < totalPages; i++) {
            dotsHtml += `<span class="attach-dot${i === 0 ? ' active' : ''}" data-page="${i}"></span>`;
        }

        const menu = document.createElement('div');
        menu.id = 'chat-attach-menu';
        menu.className = 'chat-attach-menu chat-attach-menu-grid';
        menu.innerHTML = `
            <div class="attach-pages-wrapper">
                <div class="attach-pages">${pagesHtml}</div>
            </div>
            <div class="attach-dots-row">${dotsHtml}</div>
        `;
        ...
        menu.style.left = Math.max(10, Math.min(rect.left - menuWidth / 2 + 18, window.innerWidth - menuWidth - 10)) + 'px';
        menu.style.bottom = Math.max(72, window.innerHeight - rect.top + 8) + 'px';

        // 翻页逻辑
        const pagesContainer = menu.querySelector('.attach-pages');
        const dots = menu.querySelectorAll('.attach-dot');
        let currentPage = 0;
        ...
        dots.forEach(d => {
            d.addEventListener('click', e => {
                e.stopPropagation();
                goToPage(Number(d.dataset.page));
            });
        });
        ...
        let touchStartX = 0;
        let touchEndX = 0;
        const wrapper = menu.querySelector('.attach-pages-wrapper');
        ...
        menu.addEventListener('click', e => {
            ...
            if (action === 'file') openAttachFilePicker();
            else if (action === 'media-upload') {
                menu.remove();
                setTimeout(function() {
                    const muInput = document.getElementById('media-upload-input');
                    if (muInput) {
                        muInput.value = '';
                        muInput.click();
                    }
            ...
            else if (action === 'link') promptLinkAndSend();
            else if (action === 'questionnaire') {
                menu.remove();
                if (window.QNR && window.QNR.openList) window.QNR.openList();
            } else if (action === 'call') {
                menu.remove();
                ...
                menu.remove();
                if (typeof window.openCheckinToPartnerModal === 'function') {
                    window.openCheckinToPartnerModal();
                }
            } else if (action === 'dnd') {
                menu.remove();
                if (window.DNDApp && typeof window.DNDApp.openDNDSettings === 'function') {
                    window.DNDApp.openDNDSettings();
                }
            } else if (action === 'food') {
                menu.remove();
                if (window.FoodApp && typeof window.FoodApp.openFoodPicker === 'function') {
                    window.FoodApp.openFoodPicker();
                } else {
                    showToast('吃什么功能加载中');
                }
                ...
                setTimeout(() => {
                    const sogouTab = document.querySelector('.combo-tab-btn[data-tab="sogou-search"]');
                    if (sogouTab) sogouTab.click();
                }, 50);
            } else if (action === 'music') {
                menu.remove();
                if (window.MusicBuddyEntry && typeof window.MusicBuddyEntry.open === 'function') {
                    window.MusicBuddyEntry.open();
                }
            } else if (action === 'draw') {
                menu.remove();
                if (window.DoodleApp && typeof window.DoodleApp.openDoodleBoard === 'function') {
                    window.DoodleApp.openDoodleBoard();
                } else {
                    showToast('涂鸦板加载中');
                }
                ...
                else if (window.CompanionApp) window.CompanionApp.open();
                else showToast('陪伴功能加载中');
            ...
            } else if (action === 'health-detect') {
                menu.remove();
                if (window.HealthApp && typeof window.HealthApp.openHealthModal === 'function') {
                    window.HealthApp.openHealthModal();
                } else {
                    showToast('健康检测功能加载中');
                }
            } else if (action === 'fruit-throw') {
                menu.remove();
                if (window.FruitApp && typeof window.FruitApp.openThrowPicker === 'function') {
                    window.FruitApp.openThrowPicker();
                } else {
                    showToast('扔水果功能加载中...');
                }
            } else if (action === 'fruit-slice') {
                menu.remove();
                if (window.FruitApp && typeof window.FruitApp.openSliceGame === 'function') {
                    window.FruitApp.openSliceGame();
                } else {
                    showToast('切水果功能加载中...');
                }
            } else if (action === 'avatar-swap') {
                menu.remove();
                if (window.AvatarSwapApp && typeof window.AvatarSwapApp.open === 'function') {
                    window.AvatarSwapApp.open();
                } else {
                    showToast('换头像功能加载中...');
                }
            } else if (action === 'desk-pet') {
                menu.remove();
                if (window.DeskPetApp && typeof window.DeskPetApp.openSettings === 'function') {
                    window.DeskPetApp.openSettings();
                } else {
                    showToast('桌宠功能加载中...');
                }
            }
