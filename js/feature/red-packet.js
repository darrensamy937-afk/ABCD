...
(function () {
    ...
    function showRedPacketSendForm() {
        ...
        submitBtn.onclick = function () {
            ...
            setTimeout(function () {
                ...
                // 独立判定退回：取消系统自动退回，改为手动退回
                // 原逻辑：20%概率退回 — 现已取消，所有红包保持 pending 等待对方领取或手动退回
                // 对方立即收取红包
    ...
    window.showRedPacketReceiveModal = function (recordId) {
        ...
        var html =
            '<div id="rp-receive-panel" style="text-align:center;position:relative;overflow:hidden;border-radius:16px;width:260px;min-height:380px;' + panelBg + 'display:flex;flex-direction:column;">' +
                ...
                '</div>' +
                // 底部按钮区域
                '<div style="padding:30px 20px 40px;display:flex;justify-content:center;align-items:center;gap:20px;' + (isOpened ? 'background:#ccc;' : 'background:#c4453c;') + '">' +
                    ...
                    (isPending ? '<button id="rp-return-btn" style="width:44px;height:44px;border-radius:50%;background:rgba(255,255,255,0.2);color:rgba(255,255,255,0.8);font-size:12px;font-weight:600;border:1.5px solid rgba(255,255,255,0.4);cursor:pointer;transition:all 0.15s;">退回</button>' : '') +
                '</div>' +
            '</div>';
        ...
        if (returnBtn && isPending) {
            returnBtn.onmouseenter = function () { this.style.transform = 'scale(1.1)'; this.style.background = 'rgba(255,255,255,0.3)'; };
            returnBtn.onmouseleave = function () { this.style.transform = 'scale(1)'; this.style.background = 'rgba(255,255,255,0.2)'; };
            returnBtn.onclick = function () {
                // 手动退回红包：金额返还发送方
                if (record.from === 'me') {
                    transferData.myBalance += record.amount;
                } else if (record.from === 'system') {
                    transferData.systemBalance += record.amount;
                }
                // 更新记录状态为已退回
    ...
    // ========== 24小时过期自动退回 — 已取消，改为手动退回 ==========

    window.checkRedPacketExpiry = function () {
        // 取消系统自动退回，红包保持 pending 状态直到对方领取或用户手动退回
        return;
    };
    ...
    window.showTransferBalanceSettings = function () {
        ...
        var overlay = document.createElement('div');
        overlay.style.cssText = 'position:fixed;top:0;left:0;right:0;bottom:0;background:rgba(0,0,0,0.5);z-index:10000;display:flex;align-items:center;justify-content:center;animation:fadeIn 0.2s;';
        overlay.onclick = function (e) { if (e.target === overlay) overlay.remove(); };
