...
function _getGroupCtx(tab) {
    ...
    if (tab === 'punctuations') {
        if (!window.PunctuationLib) return { groups: [], items: [], itemLabel: '标点符号' };
        return { groups: [], items: window.PunctuationLib.getLibrary(), itemLabel: '标点符号' };
    }
...
function _renderListContentOnly() {
    ...
    if (currentMajorTab === 'reply') {
        if (currentSubTab === 'custom') {
            ...
            renderType = 'image';
        } else if (currentSubTab === 'punctuations') {
            // 标点符号 tab：直接渲染面板
            if (window.PunctuationLib && typeof window.PunctuationLib.renderPunctuationList === 'function') {
                window.PunctuationLib.renderPunctuationList(list);
            } else {
                list.innerHTML = '<div style="padding:20px;text-align:center;color:#999;">标点符号库加载中...</div>';
            }
            return;
        }
